...
{
    clientConnectHook.Add(globals::serverGameClients);
    clientPutInServerHook.Add(globals::serverGameClients);
    clientDisconnectHook.Add(globals::serverGameClients);
    clientVoiceHook.Add(globals::serverGameClients);
    clientCommandHook.Add(globals::serverGameClients);
...
{
    clientConnectHook.Remove(globals::serverGameClients);
    clientPutInServerHook.Remove(globals::serverGameClients);
    clientDisconnectHook.Remove(globals::serverGameClients);
    clientVoiceHook.Remove(globals::serverGameClients);
    clientCommandHook.Remove(globals::serverGameClients);
...
KHook::Return<bool> PlayerManager::Hook_OnClientConnect(IServerGameClients* pThis,
                                                        CPlayerSlot slot,
                                                        const char* pszName,
                                                        uint64 xuid,
                                                        ...
                                                        CBufferString* pRejectReason)
{
    ...
    {
        Hook_OnClientDisconnect(pThis, slot, ENetworkDisconnectionReason::NETWORK_DISCONNECT_INVALID, pszName, xuid, pszNetworkID);
        Hook_OnClientDisconnect_Post(pThis, slot, ENetworkDisconnectionReason::NETWORK_DISCONNECT_INVALID, pszName, xuid, pszNetworkID);
    }
                                                             ...
                                                             CBufferString* pRejectReason)
{
    ...
    int client = slot.Get();
    bool orig_value = *reinterpret_cast<bool*>(KHook::GetOriginalValuePtr());
    CPlayer* pPlayer = &m_players[client];
...
PlayerManager::Hook_OnClientPutInServer(IServerGameClients* pThis, CPlayerSlot slot, const char* pszName, int type, uint64 xuid)
{
    ...
    if (!pPlayer->IsConnected())
    {
        ...
        if (!Hook_OnClientConnect(pThis, slot, pszName, 0, "127.0.0.1", false, new CBufferStringGrowable<255>()).ret)
        {
